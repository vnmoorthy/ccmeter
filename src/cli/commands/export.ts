...
export async function runExport(opts: ExportOpts): Promise<void> {
  ...
  else if (fmt === "ics" || fmt === "ical") body = renderICal(transformed);
  else throw new Error(`unknown format: ${fmt} (json|csv|md|ics)`);
...
/**
 * iCalendar export — one VEVENT per session. Drop the .ics into Apple
 * Calendar, Google Calendar, Outlook, etc. to see your Claude Code work as
 * actual calendar entries (useful for billable-time tracking and weekly
 * review).
 */
function renderICal(a: Analysis): string {
  const ESC = (s: string) =>
    s.replace(/\\/g, "\\\\").replace(/\n/g, "\\n").replace(/[,;]/g, (c) => `\\${c}`);
  const ICAL_TS = (ms: number) => {
    const d = new Date(ms);
  ...
  out.push("PRODID:-//ccmeter//ccmeter-export//EN");
  out.push("CALSCALE:GREGORIAN");
  out.push("X-WR-CALNAME:Claude Code sessions");
  out.push("X-WR-CALDESC:Sessions captured by ccmeter");

  for (const s of a.sessions) {
    if (s.startMs === 0 || s.endMs === 0) continue;
    out.push("BEGIN:VEVENT");
    out.push(`UID:${s.id}@ccmeter.vnmoorthy`);
    out.push(`DTSTAMP:${ICAL_TS(a.generatedAt)}`);
    out.push(`DTSTART:${ICAL_TS(s.startMs)}`);
    out.push(`DTEND:${ICAL_TS(s.endMs > s.startMs ? s.endMs : s.startMs + 60_000)}`);
    out.push(
      `SUMMARY:${ESC(`ccmeter ${fmtUSD(s.cost.totalCost)} · ${shortProj(s.projectPath)}`)}`,
    );
    const desc =
      `model: ${s.primaryModel}\n` +
      `turns: ${s.turnCount}, tool uses: ${s.toolUseCount}, shape: ${s.shape}\n` +
      `cache busts: ${s.cacheBusts.length}\n` +
      `${s.tag ? `tag: ${s.tag}\n` : ""}` +
      `cost breakdown: ` +
      `input ${fmtUSD(s.cost.inputCost)}, output ${fmtUSD(s.cost.outputCost)}, ` +
      `cache write ${fmtUSD(s.cost.cacheWriteCost)}, cache read ${fmtUSD(s.cost.cacheReadCost)}`;
    out.push(`DESCRIPTION:${ESC(desc)}`);
    if (s.tag) out.push(`CATEGORIES:${ESC(s.tag)}`);
    out.push("END:VEVENT");
  }
